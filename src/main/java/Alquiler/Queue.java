    ...
    public static String promedio() {
        ...
        while (aux != tail) {
            if (aux.getValue().getEstadoSolicitud().equalsIgnoreCase("Procedada") || aux.getValue().getEstadoSolicitud().equalsIgnoreCase("Finalizada")) {
                switch (aux.getValue().getCategoria().toUpperCase()) {
                    case "ZAFIRO":
                        cont1++;
                        total1 += aux.getValue().getPrecioTotal();
                        break;
                    case "ORO":
                        cont2++;
                        total2 += aux.getValue().getPrecioTotal();
                        break;
                    case "PLATA":
                        cont3++;
                        total3 += aux.getValue().getPrecioTotal();
                        break;
                    case "BRONCE":
                        cont4++;
                        total4 += aux.getValue().getPrecioTotal();
                        break;
                }
            }
            aux = aux.getNext();
        ...
        if (tail != null && (tail.getValue().getEstadoSolicitud().equalsIgnoreCase("Procedada") || tail.getValue().getEstadoSolicitud().equalsIgnoreCase("Finalizada"))) {
            switch (tail.getValue().getCategoria().toUpperCase()) {
                case "ZAFIRO":
                    cont1++;
                    total1 += tail.getValue().getPrecioTotal();
                    break;
                case "ORO":
                    cont2++;
                    total2 += tail.getValue().getPrecioTotal();
                    break;
                case "PLATA":
                    cont3++;
                    total3 += tail.getValue().getPrecioTotal();
                    break;
                case "BRONCE":
                    cont4++;
                    total4 += tail.getValue().getPrecioTotal();
                    break;
            }
        }

        promedios = String.format(
                "\n\nZAFIRO\nPromedio: %.2f\nTotal pagado: %.2f"
                + "\n\nORO\nPromedio: %.2f\nTotal pagado: %.2f"
                + "\n\nPLATA\nPromedio: %.2f\nTotal pagado: %.2f"
                + "\n\nBRONCE\nPromedio: %.2f\nTotal pagado: %.2f",
                cont1 > 0 ? total1 / cont1 : 0, total1,
                cont2 > 0 ? total2 / cont2 : 0, total2,
                cont3 > 0 ? total3 / cont3 : 0, total3,
                cont4 > 0 ? total4 / cont4 : 0, total4
        );
